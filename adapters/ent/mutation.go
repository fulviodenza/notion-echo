...
// NotionToken returns the value of the "notion_token" field in the mutation.
func (m *UserMutation) NotionToken() (r string, exists bool) {
	v := m.notion_token
	if v == nil {
		return
	}
...
// OldNotionToken returns the old "notion_token" field's value of the User entity.
// If the User object wasn't provided to the builder, the object is fetched from the database.
// An error is returned if the mutation operation is not UpdateOne, or the database query fails.
func (m *UserMutation) OldNotionToken(ctx context.Context) (v string, err error) {
	if !m.op.Is(OpUpdateOne) {
		return v, errors.New("OldNotionToken is only allowed on UpdateOne operations")
	}
	if m.id == nil || m.oldValue == nil {
		return v, errors.New("OldNotionToken requires an ID field in the mutation")
	}
	oldValue, err := m.oldValue(ctx)
	if err != nil {
		return v, fmt.Errorf("querying old value for OldNotionToken: %w", err)
	}
	return oldValue.NotionToken, nil
}

// ResetNotionToken resets all changes to the "notion_token" field.
func (m *UserMutation) ResetNotionToken() {
	m.notion_token = nil
}
